...
class ChatSession:
    def __init__(self, system_prompt=None):
        ...
        # 深度集成自定义prompt[1,2](@ref)
        if system_prompt:
            self._add_message("system", system_prompt)  # 作为系统级提示

    def generate_prompt(self, user_input):
        """动态构建提示模板（参考网页3/4的输入处理）"""
        return f"""
            角色扮演指令：纯美骑士银枝模式
            请严格遵循以下设定与我对话，用第一人称视角演绎角色，无需声明身份转换
    ...
    def _send_request(self):
        ...
        data = {
            "model": MODEL_NAME,
            "messages": self.history[-5:],  # 保留最近5轮对话
            "temperature": 0.7,
            "max_tokens": 1024,
            "top_p": 0.9
        }
...
def main():
    ...
    while True:
        try:
            ...
            if not user_input:
                print("⚜️ 愿沉默亦能传达您对美的追求...")
                continue

            if user_input in ["再见", "exit"]:
                print("\n🌹 愿伊德莉拉的辉光永驻君心，告辞！")
                break

            response = session.call_api(user_input)
